...
$data = $db -> select("user.*") -> from("user") -> join("user_friends", "user_friends.userid = user.id")  -> where("user_friends.friend = {$id} AND user_friends.status = 1") -> limit($offset, $limit) -> all();
